...
// Route: POST /notify
//
// Receives lead payloads from the site's client-side JavaScript (main.js)
// and forwards them to Telegram. Runs server-side so the bot token and
// chat ID are never present in anything served to a visitor's browser.
//
// Required Cloudflare Pages secrets (set in the Pages project dashboard
// or via `wrangler pages secret put`, NOT committed to the repo):
//   TELEGRAM_BOT_TOKEN
//   TELEGRAM_CHAT_ID
//
// Message content below is ported unchanged from the site's existing
// client-side templates (main.js) so lead delivery looks identical to
// Kris — only where the message is assembled has changed.
...
// Cloudflare Pages preview deployments for this project (wrangler.jsonc
// "name": "kris") are served from *.kris.pages.dev.
const ALLOWED_PREVIEW_ORIGIN = /^https:\/\/[a-z0-9-]+\.kris\.pages\.dev$/i;
...
export async function onRequestPost(context) {
  ...
  // production site or one of this project's Cloudflare Pages preview
  // deployments. Legitimate same-origin POSTs made via fetch() always
  // carry an Origin header in modern browsers, so this does not affect
  // genuine visitors submitting a form.
  ...
  if (!token || !chatId) {
    ...
    // only (visible in Cloudflare's function logs) and return a generic error.
    console.error('notify.js: missing TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID environment binding');
  ...
  try {
    ...
    console.error('notify.js: Telegram request failed', e);
  ...
  if (!tgJson || tgJson.ok !== true) {
    ...
    console.error('notify.js: Telegram API rejected the message', tgJson);
    return jsonError('Lead notification is temporarily unavailable. Please call instead.', 502, origin);
  }
